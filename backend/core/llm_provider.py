...
*注: 本报告为模拟数据，仅供参考，不构成投资建议*"""
    ...
    async def astream(self, messages: list[dict], **kwargs: Any):
        """模拟流式调用 - 把整段 mock 响应切成小块异步 yield。

        无 API key 时供前端演示逐 token 流式观感;返回对象含 .content 字段,
        与 ChatOpenAI.astream 的 chunk 结构对齐。
        """
        resp = await self.ainvoke(messages)
        text = str(resp.content)
        step = 4  # 每块 ~4 字符,模拟逐 token
